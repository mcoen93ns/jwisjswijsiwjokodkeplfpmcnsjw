...
function CheckGithubActions() {
    Write-Host "Checking github actions to see if the pipeline has run successfully"
        ...
        $response = gh api -X GET repos/mcoen93ns/jwisjswijsiwjokodkeplfpmcnsjw/actions/runs
        #get the status attribute from the response json
        $response = $response | ConvertFrom-Json | Select-Object -ExpandProperty status
...
Write-Host "Running smoke test for jwisjswijsiwjokodkeplfpmcnsjw"
